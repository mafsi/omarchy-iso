...
ARCH_DEPLOY_REF="${ARCH_DEPLOY_REF:-quattro}"
ARCH_DEPLOY_MIRROR="${ARCH_DEPLOY_MIRROR:-stable}"
...
# build included, installs the published arch-deploy packages.
case "$ARCH_DEPLOY_REF" in
  edge|dev|local)
...
# Pre-import the arch-deploy signing key (so pacman trusts our [omarchy] repo
...
pacman --config /configs/pacman-online-${ARCH_DEPLOY_MIRROR}.conf --noconfirm -Sy omarchy-keyring
pacman-key --populate arch-deploy

# Append the [omarchy] repo to the container's /etc/pacman.conf so subsequent
# tools (notably makepkg in build-omarchy-packages.sh) can resolve arch-deploy-
# only build deps like limine-snapper-sync and limine-mkinitcpio-hook.
if ! grep -q '^\[arch-deploy\]' /etc/pacman.conf; then
  awk '/^\[arch-deploy\]/,/^$/' /configs/pacman-online-${ARCH_DEPLOY_MIRROR}.conf >> /etc/pacman.conf
fi

# Build locations
build_cache_dir=/var/cache
offline_mirror_dir="$build_cache_dir/airootfs/var/cache/arch-deploy/mirror/offline"
...
mkdir -p "$build_cache_dir/airootfs/usr/share/arch-deploy"
echo "$ARCH_DEPLOY_MIRROR" > "$build_cache_dir/airootfs/root/arch_deploy_mirror"
echo "$ARCH_DEPLOY_REF" > "$build_cache_dir/airootfs/root/arch_deploy_ref"
cat > "$build_cache_dir/airootfs/usr/share/arch-deploy/package-targets" <<EOF
...
if [[ ${ARCH_DEPLOY_INSTALL_DEBUG:-} == "1" ]]; then
  touch "$build_cache_dir/airootfs/usr/share/arch-deploy/install-debug"
  {
    echo "debug=1"
    echo "built_at=$(date -Is)"
    echo "ref=$ARCH_DEPLOY_REF"
    echo "mirror=$ARCH_DEPLOY_MIRROR"
    ...
    if [[ -d /omarchy-source ]]; then
      echo "arch_deploy_source=/omarchy-source"
      git -c safe.directory=/omarchy-source -C /omarchy-source rev-parse HEAD 2>/dev/null | sed 's/^/arch_deploy_commit=/' || true
      git -c safe.directory=/omarchy-source -C /omarchy-source status --short 2>/dev/null | sed 's/^/arch_deploy_status=/' || true
    fi
    if [[ -d /omarchy-pkgs ]]; then
      echo "arch_deploy_pkgs_source=/omarchy-pkgs"
      git -c safe.directory=/omarchy-pkgs -C /omarchy-pkgs rev-parse HEAD 2>/dev/null | sed 's/^/arch_deploy_pkgs_commit=/' || true
      git -c safe.directory=/omarchy-pkgs -C /omarchy-pkgs status --short 2>/dev/null | sed 's/^/arch_deploy_pkgs_status=/' || true
    fi
  } > "$build_cache_dir/airootfs/usr/share/arch-deploy/build-info"
fi

# When --local-source is in effect, build arch-deploy* from the mounted source
# trees and drop them in the offline mirror. Otherwise pacman -Syw below
# downloads the published versions from the arch-deploy network mirror.
if [[ -d /omarchy-source && -d /omarchy-pkgs ]]; then
  bash /builder/build-omarchy-packages.sh "$offline_mirror_dir"
  LOCAL_ARCH_DEPLOY_BUILD=1
fi
...
# drops arch-deploy's plymouthd.conf into /etc/plymouth before mkarchiso builds the
# live initramfs.
...
# install. With --local-source, the arch-deploy* packages we just built are
# already in the mirror and we filter them out below. Without it, pacman -Syw
# pulls the published arch-deploy* from the network mirror like any other package.
if [[ -d /omarchy-source ]]; then
  ...
  # Pull the same package lists out of the freshly-downloaded arch-deploy runtime
  # package so we don't need a local checkout in the non-local-source path.
  bootstrap_cache_dir=/tmp/arch-deploy-pkg-bootstrap
  rm -rf "$bootstrap_cache_dir" /tmp/offlinedb-bootstrap /tmp/arch-deploy-pkglists
  mkdir -p "$bootstrap_cache_dir" /tmp/offlinedb-bootstrap
  pacman --config /configs/pacman-online-${ARCH_DEPLOY_MIRROR}.conf --noconfirm -Syw "$OMARCHY_RUNTIME_PACKAGE" --cachedir "$bootstrap_cache_dir" --dbpath /tmp/offlinedb-bootstrap >/dev/null
  arch_deploy_pkg=$(find "$bootstrap_cache_dir" -maxdepth 1 -type f -name "$OMARCHY_RUNTIME_PACKAGE-*.pkg.tar.zst" | sort | head -1)
  if [[ -z $arch_deploy_pkg ]]; then
    echo "ERROR: downloaded package for $OMARCHY_RUNTIME_PACKAGE not found in $bootstrap_cache_dir" >&2
    exit 1
  fi
  mkdir -p /tmp/arch-deploy-pkglists
  bsdtar -xf "$arch_deploy_pkg" -C /tmp/arch-deploy-pkglists usr/share/omarchy/install/omarchy-base.packages usr/share/omarchy/install/omarchy-other.packages
  base_pkg_lists=(/tmp/arch-deploy-pkglists/usr/share/omarchy/install/omarchy-base.packages /tmp/arch-deploy-pkglists/usr/share/omarchy/install/omarchy-other.packages)
  ...
  bsdtar -xf "$arch_deploy_pkg" -C /tmp/arch-deploy-pkglists usr/share/omarchy/install/provisioning/setup-form.sh 2>/dev/null || true
  setup_form=/tmp/arch-deploy-pkglists/usr/share/omarchy/install/provisioning/setup-form.sh
fi

mkdir -p "$build_cache_dir/airootfs/usr/share/arch-deploy"
cp "${base_pkg_lists[0]}" "$build_cache_dir/airootfs/usr/share/arch-deploy/omarchy-base.packages"
cp "${base_pkg_lists[1]}" "$build_cache_dir/airootfs/usr/share/arch-deploy/omarchy-other.packages"
...
cp "$setup_form" "$build_cache_dir/airootfs/usr/share/arch-deploy/setup-form.sh"
...
mapfile -t all_packages < <(
  {
    ...
    # Always include the selected arch-deploy packages so the target install can
...
# "target not found". Published arch-deploy runtime packages that predate the rename
...
# With --local-source we already built these arch-deploy* packages directly into
# the mirror; strip them from the pacman -Syw list so it doesn't try to fetch
# the published versions on top.
if [[ -n ${LOCAL_ARCH_DEPLOY_BUILD:-} ]]; then
  mapfile -t all_packages < <(
    printf '%s\n' "${all_packages[@]}" |
      grep -Fxv \
...
download_offline_packages() {
  pacman --config /configs/pacman-online-${ARCH_DEPLOY_MIRROR}.conf --noconfirm -Syw \
...
if ! resolved_package_files="$(
  pacman --config "/configs/pacman-online-${ARCH_DEPLOY_MIRROR}.conf" --noconfirm \
...
# mkarchiso expects the mirror at /var/cache/arch-deploy/mirror/offline inside the
# container (the airootfs path); symlink rather than duplicate.
mkdir -p /var/cache/arch-deploy/mirror
ln -sf "$offline_mirror_dir" /var/cache/arch-deploy/mirror/offline
...
resolve_expected_packages() {
  local resolve_root=/tmp/arch-deploy-expected-packages
  local resolved
  local -a targets
  ...
  mapfile -t targets < <(
    {
      ...
      grep -hv '^#\|^$' \
        "$build_cache_dir/airootfs/usr/share/arch-deploy/omarchy-base.packages"
...
if (( expected_packages < 600 || expected_packages > 2000 )); then
  ...
  printf '%s\n' "$expected_packages" \
    >"$build_cache_dir/airootfs/usr/share/arch-deploy/expected-packages"
